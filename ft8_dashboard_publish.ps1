...
<head>
  ...
  <style>
    :root {
      ...
      --text:#fc9; --text-muted:#caa; --accent:#fc6;
      --green:#5c6; --yellow:#e0b040; --red:#c55;
      --oc:#f96; --lc:#c9c; --pc:#99f; --sc:#c67; --icc:#9cf;
    }
    * { box-sizing:border-box; margin:0; padding:0; }
    body {
      font-family:'Antonio','Arial Narrow','Roboto Condensed',sans-serif;
      background:var(--bg); color:var(--text); line-height:1.4; letter-spacing:.04em;
    }
    .lcars { max-width:1600px; }
    .container { max-width:none; margin:0; padding:14px 2px; }

    header {
      display:flex; justify-content:space-between; align-items:flex-end; flex-wrap:wrap;
      gap:14px; margin-bottom:16px; background:var(--accent); color:#000;
      border-radius:0 var(--rad,32px) 0 0; padding:14px 22px 12px;
    }
    .title-section h1 { font-size:1.7rem; font-weight:700; color:#000;
      text-transform:uppercase; letter-spacing:.06em; }
    .title-section h1 .call { color:#a30000; }
    .title-section .subtitle { color:#3a1500; font-size:.85rem; margin-top:2px; font-weight:600; }
    .title-section .subtitle a { color:#701020; text-decoration:none; font-weight:700; }
    .title-section .subtitle a:hover { text-decoration:underline; }

    .stats-row { display:flex; gap:10px; flex-wrap:wrap; }
    .stat-box {
      background:#000; border:none; border-left:8px solid var(--oc);
      border-radius:8px; padding:8px 18px; text-align:right; min-width:92px;
    }
    .stat-box:nth-child(2){ border-left-color:var(--pc); }
    .stat-box:nth-child(3){ border-left-color:var(--lc); }
    .stat-box .value { font-size:1.7rem; font-weight:700; color:var(--accent); }
    .stat-box .label { font-size:.7rem; color:var(--text-muted); text-transform:uppercase; letter-spacing:.1em; }

    .tabs { display:flex; gap:8px; margin-bottom:18px; flex-wrap:wrap; }
    .tab {
      padding:8px 20px; background:var(--lc); border:none; border-radius:16px;
      color:#000; cursor:pointer; font-family:inherit; font-size:.85rem; font-weight:600;
      text-transform:uppercase; letter-spacing:.05em; transition:background .15s;
    }
    .tab:hover { background:var(--icc); }
    .tab.active { background:var(--accent); color:#000; }

    .panel { display:none; } .panel.active { display:block; }

    #map { height:500px; border-radius:12px; border:2px solid var(--border); margin-bottom:20px; }

    .map-controls, .filter-controls { display:flex; gap:12px; margin-bottom:16px; flex-wrap:wrap; align-items:center; }
    .map-controls label, .filter-controls label { font-size:.8rem; color:var(--text-muted);
      text-transform:uppercase; letter-spacing:.08em; }
    .map-controls select, .filter-controls select {
      background:#111; border:1px solid var(--border); border-radius:14px; padding:6px 14px;
      color:var(--text); font-family:inherit; font-size:.85rem;
    }

    .filter-btn {
      padding:6px 14px; background:var(--pc); border:none; border-radius:14px; color:#000;
      font-family:inherit; font-size:.78rem; font-weight:600; text-transform:uppercase;
      cursor:pointer; transition:background .15s;
    }
    .filter-btn:hover { background:var(--icc); }
    ...
    .prop-grid { display:grid; grid-template-columns:60px repeat(24,1fr); gap:2px; font-size:.7rem; margin-bottom:20px; }
    .prop-cell { aspect-ratio:1; display:flex; align-items:center; justify-content:center; border-radius:3px; }
    .prop-label { display:flex; align-items:center; justify-content:flex-end; padding-right:8px; font-weight:600; }
    .prop-hour { text-align:center; color:var(--text-muted); font-size:.65rem; }

    .psk-section { display:grid; grid-template-columns:1fr 1fr; gap:16px; }
    @media (max-width:900px){ .psk-section{ grid-template-columns:1fr; } }
    .psk-card { background:var(--card-bg); border:none; border-left:8px solid var(--icc); border-radius:12px; padding:16px; }
    .psk-card h3 { font-size:1rem; margin-bottom:12px; color:var(--accent); text-transform:uppercase; letter-spacing:.05em; }
    .psk-card h3 .count { font-weight:normal; color:var(--text-muted); }
    .spot-list { max-height:400px; overflow-y:auto; }
    .spot { display:grid; grid-template-columns:1fr auto auto auto; gap:12px; align-items:center;
      padding:8px 0; border-bottom:1px solid var(--border); font-size:.85rem; }
    .spot:last-child { border-bottom:none; }
    .spot .call { font-weight:600; color:var(--accent); }
    .spot .info { color:var(--text-muted); }
    .spot .country { font-size:.8rem; }
    .spot .flag { font-size:1.1rem; margin-right:4px; }
    .spot .snr { font-weight:600; }
    .spot .snr.good{ color:var(--green); } .spot .snr.ok{ color:var(--yellow); } .spot .snr.weak{ color:var(--red); }

    .qso-table-wrap { overflow-x:auto; border:none; border-radius:12px; background:var(--card-bg); }
    .qso-table { width:100%; border-collapse:collapse; font-size:.85rem; }
    .qso-table th, .qso-table td { padding:10px 14px; text-align:left; border-bottom:1px solid var(--border); }
    .qso-table th { background:#000; font-weight:700; color:var(--accent); text-transform:uppercase; font-size:.72rem; letter-spacing:.08em; }
    .qso-table tr:hover { background:rgba(255,204,102,.06); }
    .qso-table .call { color:var(--accent); font-weight:600; }
    .qso-table .flag { font-size:1rem; margin-right:6px; }

    .empty-state { text-align:center; padding:60px 20px; color:var(--text-muted); text-transform:uppercase; letter-spacing:.06em; }
    .empty-state h3 { color:var(--text); margin-bottom:8px; }
    .empty-state p { max-width:400px; margin:0 auto; }

    footer {
      margin-top:30px; background:var(--sc); color:#000;
      border-radius:0 0 var(--rad,32px) var(--rad,32px);
      text-align:right; font-size:.78rem; padding:10px 22px; font-weight:600;
      text-transform:uppercase; letter-spacing:.08em;
    }
    footer a { color:#3a1500; text-decoration:none; font-weight:700; }
...
<div class="lcars">
  <div class="rail">
    ...
    <a class="blk i" href="https://www.qrz.com/db/$MyCallsign">QRZ<small>$MyCallsign</small></a>
    <div class="railfill"></div>
    <a class="blk g" href="../sstv/rx/">SSTV &#9656;<small>Gallery</small></a>
  </div>
  <div class="col">
  <div class="container">
    <header>
  ...
  <script>
    ...
    function initMap() {
      ...
      L.circleMarker([MY_LAT, MY_LON], {
        radius: 10, fillColor: '#ff9966', color: '#fff',
